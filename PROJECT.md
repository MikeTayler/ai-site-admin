...
- **Site preview & history (phase 4.6)** — `/dashboard/preview`: resizable iframe of the live Vercel URL (from latest `Change.deployUrl` or Vercel API), desktop/tablet/mobile widths, manual refresh, and auto-refresh after a successful pipeline run (SSE `result` with `deployUrl`). On **Chat**, the same preview is available as a collapsible side panel with a drag handle. `/dashboard/history` lists `Change` rows as a timeline (prompt, summary, commit link, **Restore this version** via `revertToCommit`).
